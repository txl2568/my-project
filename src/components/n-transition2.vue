		...
		<div class="main" :style="{transform: `translateY(${transY}rpx)`,'transition-timing-function': speed,'height': height+'rpx'}" @transitionend="onTransitionend">
			<slot>
                <div style="width:98%">
                    <p >配置模式</p>
                    <div class="list">
                         <div class="btn" v-for="(item,i) in data" :key="i" @click="selectItem2(item,i)"  :style="{'background':activeindex2===i?'#1890ff':'','color':activeindex2===i?'#fff':'#999'}">{{item.label}}</div>
                    </div>
                    <div class="set-btn">
