<template>
	<div  :class="{'uni-scroll-tab': scroll === true }" class="uni-tab">
		<div
		...
		class="uni-tab-item">
			...
			<span>{{rangeKey == '' ? tab : tab[rangeKey]}}</span>
		</div>
		<div v-if="!scroll" :style="[{ right: barRight + '%', left : barLeft + '%', borderColor: activeColor }]" class="uni-tab-bar" :class="back ? 'uni-tab-bar-backward' : 'uni-tab-bar-forward'"></div>
	</div>
</template>
<script>
	export default {
		...
		methods: {
			itemClick(index,tab){
				...
				sessionStorage.setItem('isNowTab', this.isNowTab);
