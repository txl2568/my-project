            ...
            <div class="card">
                ...
                <div class="model-img">
                    <div class="border-img" >
                        <img  :src="'http://coverimage.oss-cn-hangzhou.aliyuncs.com/image/'+coverImgpath" v-show="!localIds"  @click="goxiugai('img11')" />
                        <img  :src="localIds"  @click="goxiugai('img11')"  v-show="localIds"  />
                        <!-- <i class="iconfont icon-xiangji" style=" font-size: 70px;color: #666;"></i> -->
                    </div>
                    <div class="border-img" >
                        <img  :src="'http://coverimage.oss-cn-hangzhou.aliyuncs.com/image/'+coverImgpath2" v-show="!localIds2" @click="goxiugai('img12')" />
                        <img  :src="localIds2"  @click="goxiugai('img12')"  v-show="localIds2"  />
                    </div>
                </div>
            </div>
...
import { selectIsNotInstallByMcnOrSnOwner, selectDeptName, installEntryOwner } from "../api/user/function";
    ...
    methods:{
        ...
        goSearch(){
            ...
            if(!this.ChipCode){
                ...
                selectIsNotInstallByMcnOrSnOwner(this.ChipCode).then(res =>{  
        ...
        goBind(){
            ...
            if((!this.coverImgpath || !this.coverImgpath2)&&(!this.serverId || !this.serverId2)){
                this.$refs.iosDialog2.style.display = 'block';
                this.message = '请上传两张图片'
                return
            }
        ...
        installEntryOwner(){
                var param = {
                    "coverImgpath": this.serverId?this.serverId:this.coverImgpath,
                    "coverImgpath2":  this.serverId2?this.serverId2:this.coverImgpath2,
                    "installMcn": this.coverMcn,
                    "installRearDeptName": this.deptName?this.deptName:'',
                    "installRearLat": this.latitude,
                    "installRearLong": this.longitude,
                }
                installEntryOwner(param).then(res =>{
                    this.$refs.Toast2.style.display = 'none';
                    if (res.data.code == 200) {
                        this.$refs.iosDialog2.style.display = 'block';
                        this.message = '安装录入成功！';
